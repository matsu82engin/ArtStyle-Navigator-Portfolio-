...
RSpec.describe 'Posts API', type: :request do
  describe 'POST /api/v1/posts' do
    ...
    # ここから無効なパラメータのテスト
    context 'with invalid title' do # 無効なパラメータ
      ...
      context 'with invalid request structure or parameters' do # 無効なリクエスト構造またはパラメータ
        context 'when post_images_attributes is empty' do # (画像1枚以上必須)
          it 'returns 422 Unprocessable Entity' do # 画像がなければエラー
        ...
        context 'when post_images_attributes has invalid format (not an array)' do # post_images_attributes が配列でない場合
          ...
          it 'returns 400 Bad Request when post_images_attributes is not an array' do # post_images_attributes が期待する形式(配列)になっていない場合
      ...
      context 'when a alt' do # alt ついて
        ...
        # TODO 後で caption と alt が同じ値になっているかもチェックすべき
      end
    end
  end
end
